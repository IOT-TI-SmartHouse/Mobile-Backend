...
function getCurrentValue(instance, sequelize){
    ...
    return SensorData
        ...
        .then(function(){
            return sequelize.query("SELECT settings.typeId, settings.minimum, settings.maximum FROM settings WHERE settings.typeId = " + instance.dataValues.typeId + " AND settings.userId = " + Global.userId + " AND settings.departmentId = (SELECT departmentId FROM sensornodes WHERE id = " + instance.dataValues.sensorNodeId + ");", { type:Sequelize.QueryTypes.SELECT})
        })
