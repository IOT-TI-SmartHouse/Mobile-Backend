...
function getStatus(instance, sequelize) {
  ...
  return sequelize
    .query(
      `SELECT settings.typeId, settings.minimum, settings.maximum, settings.departmentId
        FROM settings WHERE settings.userId = ` +
        Global.userId +
        ` AND settings.departmentId IN (SELECT id FROM Departments
        WHERE greenhouseId = ` +
    ...
    .then(function() {
      ...
      return sequelize.query(
        `SELECT s.typeId, t.type, t.name, t.unit, ROUND( AVG(value),2 ) AS average, d.id AS departmentId
            FROM sensors s
            LEFT JOIN types t ON t.id = s.typeId
            LEFT JOIN sensordata sd ON sd.sensorId = s.id
            LEFT JOIN sensornodes sn ON sn.id = s.sensorNodeId
            LEFT JOIN departments d ON d.id = sn.departmentId
            WHERE s.sensorNodeId IN (SELECT id FROM sensornodes WHERE departmentId IN (SELECT id FROM Departments WHERE greenhouseID = ` +
          instance.dataValues.id +
          `))
            GROUP BY sn.departmentId, s.typeId;`,
        { type: Sequelize.QueryTypes.SELECT }
      );
    })
    .then(function(results) {
    ...
    .then(function() {
      return sequelize.query(
        `SELECT s.typeId, t.type, t.name, t.unit, ROUND( AVG(value),2 ) AS average
            FROM sensors s
            LEFT JOIN types t ON t.id = s.typeId
            LEFT JOIN sensordata sd ON sd.sensorId = s.id
            WHERE s.sensorNodeId IN (SELECT id FROM sensornodes WHERE departmentId IN (SELECT id FROM Departments WHERE greenhouseId = ` +
